...
public class AppDao {
  ...
  public Observable<RxModelWithSy> MainInit() {
    Observable<RxModelWithSy> observable =
        Observable.create(new ObservableOnSubscribe<RxModelWithSy>() {
          @Override public void subscribe(ObservableEmitter<RxModelWithSy> subscriber)
              throws Exception {
            String time =
                (String) SpUtils.getParam(App.getAppContext(), Constant.SLEEP_TIME_KEY, "");
   ...
   *  睡眠Action
   * @param mData
   * @param mOtherAdapterData
   * @param mConsumer
   */
  public void doSleepApp(List<AppInfo> mData, List<AppInfo> mOtherAdapterData,
      Consumer<RxModelWithSy> mConsumer) {
    Observable.create(new ObservableOnSubscribe<RxModelWithSy>() {
      @Override public void subscribe(ObservableEmitter<RxModelWithSy> subscriber)
          throws Exception {
        //找出已经解冻的app进行冻结
        int mWarnApp = findWarnApp(mData, mOtherAdapterData);
        RxModelWithSy rxModelWithSy = new RxModelWithSy(mWarnApp, 0, mData, mOtherAdapterData);
        subscriber.onNext(rxModelWithSy);
        subscriber.onComplete();
      }
    }).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(mConsumer);
  }
   ...
   * @param mConsumer
   */
  public void doWarnApp(AppInfo mAppInfo,Consumer<AppInfo> mConsumer){
  ...
  private void WarnAppAction(AppInfo mAppInfo) {
   ...
   * @param mConsumer
   */
  public void doSearchApp(String str,List<AppInfo> mListData,Consumer<AppInfo> mConsumer){
    Observable.create(new ObservableOnSubscribe<AppInfo>() {
      @Override public void subscribe(ObservableEmitter<AppInfo> subscriber) throws Exception {
